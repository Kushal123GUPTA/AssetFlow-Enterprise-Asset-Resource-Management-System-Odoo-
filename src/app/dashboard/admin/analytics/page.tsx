...
import { BarChartOutlined, LineChartOutlined, AlertOutlined, TrophyOutlined, FireOutlined } from "@ant-design/icons";
import {
...
import BookingHeatmap from "@/app/modules/reports/components/BookingHeatmap";
import AssetsDueList from "@/app/modules/reports/components/AssetsDueList";
...
export default function AnalyticsPage() {
  ...
  const [lifecycle, setLifecycle] = useState({ dueMaintenance: [], nearingRetirement: [] });
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const fetchAnalytics = async () => {
      try {
        const [utilRes, maintRes, assetsRes, heatRes, lifeRes] = await Promise.all([
          ...
          fetch("/api/modules/reports/routes/analytics/heatmap").then((r) => r.json()),
          fetch("/api/modules/reports/routes/analytics/lifecycle").then((r) => r.json()),
        ]);
        ...
        setLifecycle(lifeRes.data || { dueMaintenance: [], nearingRetirement: [] });
      } catch (error) {
        console.error(error);
  ...
  return (
    <PageShell>
      <PageHeader
        ...
        description="Utilization, maintenance frequency, booking peaks, and asset lifecycles"
        actions={
          <ReportExportActions
          ...
          <div className="min-h-0 w-full flex-1">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={maintenance} margin={{ top: 5, right: 30, left: -20, bottom: 5 }}>
                ...
                <Line type="monotone" dataKey="requests" name="Requests" stroke="#ff6b00" strokeWidth={3} dot={{ r: 4, fill: "#ff6b00" }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </Card>
      ...
      <Card>
        <h2 className="mb-4 flex items-center gap-2 text-lg font-bold text-gray-100">
      ...
      <Card>
        <h2 className="mb-4 text-lg font-bold text-gray-100">Maintenance & retirement watchlist</h2>
        <AssetsDueList
          dueMaintenance={lifecycle.dueMaintenance}
          nearingRetirement={lifecycle.nearingRetirement}
        />
      </Card>
